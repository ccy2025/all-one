...
namespace All_in_1
{
    internal class Appdata_2_Excel_Device_Lite
    {
        public void Run()
        {
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine("\nAppdata生成Excel - Device Lite");
            Console.ResetColor();
        ...
        static void ReadXml(string xmlFile, string xlsxFile)
        {
            ...
            using (var workbook = new ExcelPackage())
            {
                ...
                for (int col = worksheet.Dimension.End.Column; col >= 1; col--)
                {
                    ...
                    // 遍历列的每一行，排除前两行表头
                    for (int checkRow = 3; checkRow <= worksheet.Dimension.End.Row; checkRow++)
                    {
                        // 检查单元格是否有内容
                        if (worksheet.Cells[checkRow, col].Value != null)
                        {
                            isEmpty = false;
                            break;
                        }
        ...
        static string GetOutputFilePath(string xmlFile)
        {
            ...
            string xlsxFile = Path.Combine(directory, $"{fileName}_Device.xlsx");
            return xlsxFile;
        }
    }
}