...
namespace All_in_1
{
    internal class Excel_2_Paper
    {
        public void Run()
        {
            ...
            string xmlFilePath = Path.Combine(directoryPath, Path.GetFileNameWithoutExtension(excelFilePath) + "_Paper.xml");
            ...
            using (ExcelPackage package = new ExcelPackage(new FileInfo(excelFilePath)))
            {
                ...
                for (int i = 2; i <= rowCount; i++)
                {
                    ...
                    string? questionDescription = worksheet.Cells[i, 3].Value?.ToString();
                    string? scoreConditionVerb = worksheet.Cells[i, 4].Value?.ToString();
                    string? questionValue = worksheet.Cells[i, 5].Value?.ToString();
                    string? scoreValue = worksheet.Cells[i, 6].Value?.ToString();
                    string? upperLimit = worksheet.Cells[i, 7].Value?.ToString();
                    string? lowerLimit = worksheet.Cells[i, 8].Value?.ToString();
                    string? upperMaxLimit = worksheet.Cells[i, 9].Value?.ToString();
                    string? lowerMaxLimit = worksheet.Cells[i, 10].Value?.ToString();
                    string? scoreMax = worksheet.Cells[i, 11].Value?.ToString();
                    string? questionName = worksheet.Cells[i, 12].Value?.ToString();
                    ...
                    if (!string.IsNullOrEmpty(questionName))
                    {
                        ...
                        scoreInnerVerbElement.InnerText = scoreConditionVerb;
